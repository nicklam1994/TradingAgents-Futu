...
def _deterministic_uuid(name: str, params: Dict[str, Any]) -> str:
    """Generate a deterministic UUID based on monitor name and parameters.

    This ensures the same monitor configuration always produces the same ID,
    preventing duplicate alert rules on restart.
    """
    # Sort params for consistent hashing
    param_str = "|".join(f"{k}={v}" for k, v in sorted(params.items()))
    content = f"{name}|{param_str}"
...
class VolumeSpikeMonitor(BaseMonitor):
    ...
    def to_alert_rule(self) -> AlertRule:
        ...
        rule_id = _deterministic_uuid(
            f"VolumeSpike_{self.stock_code}",
            {"lookback_days": self.lookback_days, "spike_multiplier": self.spike_multiplier},
        )
        return AlertRule(
            id=rule_id,
...
class PriceChangeMonitor(BaseMonitor):
    ...
    def to_alert_rule(self) -> AlertRule:
        ...
        rule_id = _deterministic_uuid(
            f"PriceChange_{self.stock_code}",
...
class SentimentMonitor(BaseMonitor):
    ...
    def __init__(
        ...
        # Sentiment scores range is [-1.0, 1.0], so threshold must be in (0, 1.0]
        if not (0 < sentiment_threshold <= 1.0):
            raise ValueError(
                f"sentiment_threshold must be in (0, 1.0], got {sentiment_threshold}"
            )
