...
Adapted from Vibe-Trading agent/src/shadow_account/scanner.py.
"""
...
# Type for price fetcher callback
PriceFetcher = Callable[..., Optional[pd.DataFrame]]

# Liquid baskets for HK/US markets (TAF-specific)
...
def scan_today_signals(
    ...
    *,
    ...
    Args:
        ...
        price_frames: Optional symbol -> OHLCV frame injection.
        fetcher: Optional callable used when price_frames has no symbol.

    Returns:
        Possibly-empty list of matched candidates. Research-only.
    """
    ...
    for rule in profile.rules:
        ...
        if market not in _LIQUID_BASKETS:
            continue
        basket = _LIQUID_BASKETS.get(market, [])
        lo, hi = rule.holding_days_range
        reason = f"{rule.rule_id} 价格特征匹配 (持有 {lo}-{hi}天)"
        market_count = 0
...
def _entry_condition_matches(
    ...
    # Default heuristic: momentum positive + price above MA
...
def _normalize_bars(frame: pd.DataFrame, target: date) -> Optional[pd.DataFrame]:
