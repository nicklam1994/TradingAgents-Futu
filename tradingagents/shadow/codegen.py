...
Adapted from Vibe-Trading agent/src/shadow_account/codegen.py (219 lines).
...
files and launching backtests.

TAF adaptation notes:
    - Imports from ``tradingagents.shadow.models`` (not ``src.shadow_account``).
    - Market detection limited to HK/US (no china_a / crypto).
    - ``build_config()`` adds TAF-specific YAML strategy fields.
"""
...
def build_config(
    ...
    *,
    ...
    TAF adaptation of Vibe-Trading's ``render_config()``. Adds
    ``strategy_name`` and ``shadow_id`` for TAF's strategy routing.
    ...
    cfg: dict[str, Any] = {
        ...
        "engine": "daily",
        "shadow_id": profile.shadow_id,
        "strategy_name": f"shadow_{profile.shadow_id}",
    }
    if extra:
        cfg.update(extra)
    return cfg
