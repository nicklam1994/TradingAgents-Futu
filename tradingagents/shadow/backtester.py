...
Adapted from Vibe-Trading agent/src/shadow_account/backtester.py (524 lines).

Responsibilities:
    1. Pick representative symbols per market based on the user's preferred
       markets (with a liquid-basket fallback).
    2. Render a run_dir (via ``codegen.write_run_dir``) and call an injectable
       backtest function.
...
TAF adaptation notes:
    - Only HK/US markets (no china_a / crypto).
    - ``runs_dir()`` uses ``~/.tradingagents/shadow_runs/`` (no storage module).
    - Attribution uses TAF's ``extractor.pair_trades_fifo`` for roundtrip data.
    - ``run_backtest_fn`` is fully injectable for testing.
"""
...
# TAF only supports HK and US markets.
SUPPORTED_MARKETS: tuple[str, ...] = ("HK", "US")

_LIQUID_BASKETS: dict[str, list[str]] = {
    "HK": ["HK.00700", "HK.09988", "HK.03690", "HK.00388", "HK.01810"],
    "US": ["AAPL", "MSFT", "NVDA", "AMZN", "GOOGL"],
}

# Type for the injectable backtest runner
BacktestFn = Callable[[str], str]


def runs_dir(shadow_id: str) -> Path:
    """Return the run directory for a given shadow_id.

    Uses ``~/.tradingagents/shadow_runs/<shadow_id>/``.
    """
    base = Path.home() / ".tradingagents" / "shadow_runs" / shadow_id
...
def select_multi_market_codes(
    ...
    *,
    ...
    Args:
        ...
        markets: Markets to include (HK/US only).
...
def run_shadow_backtest(
    ...
    *,
    ...
    run_backtest_fn: Optional[BacktestFn] = None,
) -> ShadowBacktestResult:
    ...
    Args:
        ...
        markets: Target market buckets (HK/US only).
        per_market_count: Codes per market.
        source: Loader source (``auto`` routes by suffix).
        initial_capital: Starting cash.
        journal_df: Optional trades DataFrame for attribution computation.
            Must have columns: symbol, datetime, side, price, qty.
            Attribution is skipped if None or empty.
        run_backtest_fn: Injection point for tests — callable(run_dir_str)
            returning a JSON string with the same shape as the backtest
            runner output. Defaults to a stub that returns an empty result.
...
def load_cached_result(shadow_id: str) -> Optional[ShadowBacktestResult]:
...
def _default_run_backtest_fn(run_dir_str: str) -> str:
    """Stub backtest runner — returns empty result.

    In production, this should be replaced with the actual backtest runner.
    The injectable ``run_backtest_fn`` parameter on ``run_shadow_backtest``
    allows callers to provide a real implementation.
    """
    return json.dumps({
        "status": "ok",
        "artifacts": {},
...
def _per_market_breakdown(
    ...
    metrics. This is faithful (same backtest) but intentionally lossy.
    """
    if not combined:
...
def _attribution_or_zero(
    *,
    ...
    """Compute attribution if the journal DataFrame is available, else zeros."""
    ...
    try:
        ...
        roundtrips = pair_trades_fifo(journal_df)
