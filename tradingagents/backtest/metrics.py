"""Shared backtest metrics — annualisation, trade statistics, and full metric calc.

Ported from Vibe-Trading agent/backtest/metrics.py (246 lines).
Adapted for TAF: market-specific annualisation (HK/US both use 252 bars/day),
source-agnostic (no tushare/yfinance/okx distinction).
...
# HK and US equity markets both have ~252 trading days per year.
# Intraday bars_per_day varies by interval; daily = 1.
_TRADING_DAYS_HK = 252
_TRADING_DAYS_US = 252

_BARS_PER_DAY = {
    "1m":  240,   # ~4h session (HK 330min ≈ 240 bars at 1min; US 390min)
    "5m":  48,
    "15m": 16,
    "30m": 8,
    "1H":  4,
...
def calc_bars_per_year(
    ...
    Returns:
        Bars per year.
    """
    trading_days = _TRADING_DAYS_HK if market == "hk" else _TRADING_DAYS_US
...
def calc_metrics(
    ...
    # Sortino — downside deviation uses only negative returns
    ...
    if bench_ret is not None and len(bench_ret) > 0:
        ...
        ir = float(
            active_ret.mean() / (active_std + 1e-10) * np.sqrt(bpy)
        )
