...
Ported from Vibe-Trading agent/backtest/engines/global_equity.py (78 lines).
Inherits from the enhanced BaseEngine which provides the bar-by-bar
execution loop, signal alignment, and metrics integration.
...
from tradingagents.backtest.base_engine import BaseEngine
...
class GlobalEquityEngine(BaseEngine):
    ...
    def calc_commission(
        ...
        ``direction`` is unused — reserved for future short-borrow fees
