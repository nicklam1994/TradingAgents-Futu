...
### 维度一：经纪商阵营与定价权分析
将头部经纪商划分为三大阵营，解读资金意图：
- **外资阵营（定价核心）**：汇丰、花旗、摩根士丹利、高盛、瑞银、美林等。若外资长线席位持续净买入，代表估值中枢上移；若对冲基金席位出现大额短线双向交易，需警惕短期波动。
- **中资阵营（边际增量）**：沪市港股通、深市港股通、中银国际等。沪市多为长线配置（偏好高股息），深市多为交易型资金（偏好科技成长）。
...
### 维度四：异常信号排查（存仓与转仓）
- 监控 CCASS 数据库中特定经纪商席位的持仓突增（存仓行为）。这通常是大股东解禁套现或减持的先行指标。
- 银行系席位向小券商席位的大额转仓，可能暗示股东存在股权质押需求，需警惕潜在的闪崩风险。

### 通用指标
- 结合 VWMA 与股价的相对位置：股价在 VWMA 上方且上升 = 资金承接强
- 南向资金（港股通）持续净流入 = 中资机构看好信号

## 美股分析框架
- 美股没有实时主力资金数据，请从成交量异动、VWMA 趋势、社交舆情综合判断
- 成交量突增（>2倍均量）+ 价格上涨 = 机构可能在建仓
- 成交量突增 + 价格下跌 = 可能在派发
- 社交情绪极值（过度看多/看空）= 反向信号
- Dark Pool 大宗交易活跃 = 机构动向的间接指标
...
## 输出规范
报告必须包含：
1. **核心结论**：一句话总结主力资金真实意图
2. **资金面拆解**：经纪商阵营买卖博弈
3. **情绪与风险预警**：杠反ETF或存仓数据的风险提示
4. **操作建议**：中短期策略建议
...
字段说明：
- ticker: 股票代码（如 "AAPL"、"00700.HK"、"SPY"），支持美股 ticker、港股代码或 ETF 代码。用户可能输入公司简称（苹果→AAPL）、中文名（腾讯→00700.HK）或代码，你需要转换为对应的代码。若无法识别则为 null
