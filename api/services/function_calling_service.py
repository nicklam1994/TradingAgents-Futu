...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "resolve_stock",
            "description": "通过名称或代码查询股票数据库（23,000+ 只港股/美股）。当用户提到可能是股票的名字但你不确定代码时，先用此工具确认。例如用户说 'SPCX'、'腾讯'、'英伟达'，先查询确认是否存在。",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "股票名称或代码，如 'SPCX'、'腾讯'、'英伟达'、'AAPL'"
                    }
                },
                "required": ["query"]
            }
...
3. **解析股票** — 使用 resolve_stock 工具，通过名称或代码查询数据库（23,000+ 只港股/美股）
4. **系统帮助** — 使用 get_help 工具

**重要规则：**
- 我们的数据库包含 23,000+ 只港股/美股。当用户提到不确定的名字时，请先用 resolve_stock 工具查询，不要假设它不存在或拼写错误。
- 例如用户说 "SPCX"，不要认为是 "SpaceX" 的拼写错误，先用 resolve_stock 查询。
- ⚠️ **歧义澄清（强制）：** 当 resolve_stock 返回多个结果（disambiguation_required=true）时，你必须：
  1. 列出所有候选股票（名称 + 代码 + 市场标志）
  2. 问用户「请问你要分析哪一个？」
  3. **绝对不能擅自选择一个开始分析**
  示例回复："找到以下相关股票：\n1. 🇭🇰 腾讯控股 00700.HK\n2. 🇺🇸 腾讯音乐 TME.US\n3. 🇭🇰 腾讯控股-R 80700.HK\n请问你要分析哪一个？"
- 只有当 resolve_stock 返回恰好 1 个结果时，才可以直接调用 analyze_stock。
- 如果用户问的是不需要调用工具的问题（如闲聊），直接用自然语言回复。
