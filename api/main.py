...
def _init_bot_manager() -> BotManager:
    ...
    if telegram_bot:
        async def _fc_disambig_callback(data: str, user_id: str, chat_id: str, message_id: str):
            ...
            if not symbol:
                return
            # Use LLM to format a rich quote card
            try:
                from tradingagents.dataflows.stock_resolver import to_display
                ...
                await telegram_bot._send_response(BotResponse(
                    text=f"✅ 已选择: {display} ({symbol})\n\n📊 正在获取行情...",
                    chat_id=str(chat_id),
                ))
                # Use function calling to get rich formatted quote
                quote_result = await _bot_function_call_fn(f"查询 {symbol} 的实时行情", user_id, str(chat_id))
                if quote_result.get("ok") and quote_result.get("data", {}).get("response"):
                    quote_text = quote_result["data"]["response"]
                    await telegram_bot._send_response(BotResponse(
